...
const register = asyncHandler(async (req, res) => {
  const { name, email, password, qualifications, bio, picture, availability } =
    req.body;
...
const login = asyncHandler(async (req, res) => {
  ...
  if (therapist && (await bcrypt.compare(password, therapist.password))) {
    res
      .cookie("token", generateToken(therapist._id), {
        path: "/",
        httpOnly: true,
...
const createSession = asyncHandler(async (req, res) => {
  ...
  if (!user || !sessionTime) {
    return res
      .status(400)
      .json({ message: "Missing required session details" });
  }
